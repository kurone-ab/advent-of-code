...
export const isInvalidId = (id) => {
  ...
  const factors = allFactorsAbove1(stringId.length);
  return factors.some(factor => {
    const partitioningArray = partition(stringId, factor);
    const first = partitioningArray[0];
    return (partitioningArray.every((it) => it === first))
  })
};
...
console.log('End in:', performance.now() - startTime, 'ms');
